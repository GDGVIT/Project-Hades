...
import (
	...
	http "github.com/GDGVIT/Project-Hades/simple_projection/pkg/http"
...
var fs = flag.NewFlagSet("simple_projection", flag.ExitOnError)
...
func Run() {
	...
	if *zipkinURL != "" {
		...
		recorder := zipkingoopentracing.NewRecorder(collector, false, "localhost:80", "simple_projection")
...
func initHttpHandler(endpoints endpoint.Endpoints, g *group.Group) {
	...
	options["ProjectAll"] = append(options["ProjectAll"], contextOptions)
	options["ProjectPresent"] = append(options["ProjectPresent"], contextOptions)
	options["ProjectAbsent"] = append(options["ProjectAbsent"], contextOptions)
