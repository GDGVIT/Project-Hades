  ...
  diff_match_patch.prototype.diff_cleanupSemanticLossless = function (a) {
    function b(a, b) {
      ...
      if (a[c - 1][0] == 0 && a[c + 1][0] == 0) {
        var d = a[c - 1][1]; var e = a[c][1]; var f = a[c + 1][1]; var g = this.diff_commonSuffix(d, e); if (g) var h = e.substring(e.length - g); var d = d.substring(0, d.length - g); var e = h + e.substring(0, e.length - g); var f = h + f; for (var g = d, h = e, j = f, i = b(d, e) + b(e, f); e.charAt(0) === f.charAt(0);) { var d = d + e.charAt(0); var e = e.substring(1) + f.charAt(0); var f = f.substring(1); const k = b(d, e) + b(e, f); k >= i && (i = k, g = d, h = e, j = f); }a[c - 1][1] != g && (g ? a[c - 1][1] = g : (a.splice(c - 1, 1), c--), a[c][1] = h, j ? a[c + 1][1] = j : (a.splice(c + 1, 1), c--));
      }c++;
    }
    ...
    for (let b = this.Match_MaxBits, c = 0; c < a.length; c++) {
      if (!(a[c].length1 <= b)) {
        const d = a[c]; a.splice(c--, 1); for (var e = d.start1, f = d.start2, g = ''; d.diffs.length !== 0;) {
          const h = new diff_match_patch.patch_obj();
          let j = !0; h.start1 = e - g.length; h.start2 = f - g.length; g !== '' && (h.length1 = h.length2 = g.length, h.diffs.push([0, g])); for (;d.diffs.length !== 0 && h.length1 < b - this.Patch_Margin;) {
